...
def manually_fill():
    ...
    SUB = tk.Label(sb, text="Enter Subject : ", width=15, height=2,
                   fg="black", bg="orange", font=('times', 15, ' bold '))
    SUB.place(x=30, y=100)

    global SUB_ENTRY

    SUB_ENTRY = tk.Entry(sb, width=20, bg="skyblue",
                         fg="black", font=('times', 23))
    SUB_ENTRY.place(x=250, y=105)

    fill_manual_attendance = tk.Button(sb, text="Fill Attendance", command=fill_attendance, fg="black", bg="#32CD32", width=20, height=2,
                                       activebackground="white", font=('times', 15, ' bold '))
...
def take_img():
    ...
    if l1 == '':
        ...
        try:
            ...
            # Set resolution and frame rate to improve performance
            cam.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
            cam.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            cam.set(cv2.CAP_PROP_FPS, 30)

            detector = cv2.CascadeClassifier(
                "C:\\Users\\BK\\Downloads\\haarcascade_frontalface_default.xml")
            Enrollment = txt.get()
            Name = txt2.get()
            sampleNum = 0
            while (True):
                ...
                if not ret:
                    ...
                    break
                # convert to grayscale and downscale image to improve detection speed
                    
                gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                gray_resized = cv2.resize(gray, (320, 240)) # Reducing size for faster processing
                faces = detector.detectMultiScale(gray_resized,scaleFactor= 1.2,minNeighbors= 5)
                for (x, y, w, h) in faces:
                    #Scale the coordinate backup to match the original image size
                    x *= 2
                    y *= 2
                    w *= 2
                    h *= 2
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                harcascadePath ="C:\\Users\\BK\\Downloads\\haarcascade_frontalface_default.xml"
    ...
    tx = tk.Entry(windo, width=20, bg="skyblue",
                  fg="black", font=('times', 23))
    tx.place(x=250, y=105)

    fill_a = tk.Button(windo, text="Fill Attendance", fg="white", command=Fillattendances, bg="#32CD32", width=20, height=2,
                       activebackground="white", font=('times', 15, ' bold '))
...
def admin_panel():
    ...
    un = tk.Label(win, text="Enter username : ", width=15, height=2, fg="black", bg="#32CD32",
                  font=('times', 15, ' bold '))
    un.place(x=30, y=50)

    pw = tk.Label(win, text="Enter password : ", width=15, height=2, fg="black", bg="#32CD32",
                  font=('times', 15, ' bold '))
    pw.place(x=30, y=150)
    ...
    un_entr = tk.Entry(win, width=20, bg="skyblue", fg="black",
                       font=('times', 23))
    ...
    pw_entr = tk.Entry(win, width=20, show="*", bg="skyblue",
                       fg="black", font=('times', 23))
    ...
    Login = tk.Button(win, text="Login", fg="black", bg="orange", width=20,
                      height=2,
                      activebackground="Red", command=log_in
, font=('times', 15, ' bold '))
...
def trainimg():
    ...
    global detector
    detector = cv2.CascadeClassifier("C:\\Users\\BK\\Downloads\\haarcascade_frontalface_default.xml")
    try:
        global faces, Id
...
message = tk.Label(window, text="FACE RECOGNITION ATTENDANCE SYSTEM", bg="#2196F3", fg="white", width=50,
...
lbl = tk.Label(window, text="Enter Enrollment : ", width=20, height=2,
               fg="black", bg="#66CDAA", font=('times', 15, 'bold'))
...
txt = tk.Entry(window, validate="key", width=20, bg="SkyBlue1",
               fg="black", font=('times', 25))
txt['validatecommand'] = (txt.register(testVal), '%P', '%d')
txt.place(x=550, y=210)

lbl2 = tk.Label(window, text="Enter Name : ", width=20, fg="black",
                bg="#66CDAA", height=2, font=('times', 15, ' bold '))
lbl2.place(x=200, y=300)

txt2 = tk.Entry(window, width=20, bg="SkyBlue1",
                fg="black", font=('times', 25))
txt2.place(x=550, y=310)

clearButton = tk.Button(window, text="Clear", command=clear, fg="black", bg="#BFBC8F",
                        width=10, height=1, activebackground="white", font=('times', 15, ' bold '))
clearButton.place(x=950, y=210)

clearButton1 = tk.Button(window, text="Clear", command=clear1, fg="black", bg="#BFBC8F",
                         width=10, height=1, activebackground="white", font=('times', 15, ' bold '))
clearButton1.place(x=950, y=310)

AP = tk.Button(window, text="Check Registered students", command=admin_panel, fg="black",
               bg="#32CD32", width=19, height=1, activebackground="white", font=('times', 15, ' bold '))
AP.place(x=990, y=410)

takeImg = tk.Button(window, text="Take Images", command=take_img, fg="black", bg="orange",
                    width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images", fg="black", command=trainimg, bg="orange",
                     width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendance", fg="black", command=subjectchoose,
               bg="orange", width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
FA.place(x=690, y=500)

JM = tk.Button(window, text="College of engineering \n attingal", fg="black", command=admin_panel,
               bg="orange", width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
JM.place(x=90, y=380)
